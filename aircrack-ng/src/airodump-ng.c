...
int dump_write_M4( void )
{
    ...
    fseek( G.f_M4, 0, SEEK_SET );

    fprintf( G.f_M4,
            "\r\nBSSID, First time seen, Last time seen, channel, Speed, "
            "Privacy, Cipher, Authentication, Power, # beacons, # IV, LAN IP, ID-length, ESSID,  IV/s, Key\r\n" );
    ...
    while( ap_cur != NULL )
    {
        ...
        fprintf( G.f_M4, "%02X:%02X:%02X:%02X:%02X:%02X, ",
        ...
        fprintf( G.f_M4, "%04d-%02d-%02d %02d:%02d:%02d, ",
        ...
        fprintf( G.f_M4, "%2d, %3d, ",
                 ap_cur->channel,
                 ap_cur->max_speed );

        if( (ap_cur->security & (STD_OPN|STD_WEP|STD_WPA|STD_WPA2)) == 0) fprintf( G.f_M4, "    " );
        else
        {
            if( ap_cur->security & STD_WPA2 ) fprintf( G.f_M4, "WPA2" );
            if( ap_cur->security & STD_WPA  ) fprintf( G.f_M4, "WPA " );
            if( ap_cur->security & STD_WEP  ) fprintf( G.f_M4, "WEP " );
            if( ap_cur->security & STD_OPN  ) fprintf( G.f_M4, "OPN " );
        }

        fprintf( G.f_M4, ",");

        if( (ap_cur->security & (ENC_WEP|ENC_TKIP|ENC_WRAP|ENC_CCMP|ENC_WEP104|ENC_WEP40)) == 0 ) fprintf( G.f_M4, "       ");
        else
        {
            if( ap_cur->security & ENC_CCMP   ) fprintf( G.f_M4, " CCMP");
            if( ap_cur->security & ENC_WRAP   ) fprintf( G.f_M4, " WRAP");
            if( ap_cur->security & ENC_TKIP   ) fprintf( G.f_M4, " TKIP");
            if( ap_cur->security & ENC_WEP104 ) fprintf( G.f_M4, " WEP104");
            if( ap_cur->security & ENC_WEP40  ) fprintf( G.f_M4, " WEP40");
            if( ap_cur->security & ENC_WEP    ) fprintf( G.f_M4, " WEP");
        }

        fprintf( G.f_M4, ",");

        if( (ap_cur->security & (AUTH_OPN|AUTH_PSK|AUTH_MGT)) == 0 ) fprintf( G.f_M4, "   ");
        else
        {
            if( ap_cur->security & AUTH_MGT   ) fprintf( G.f_M4, " MGT");
            ...
            if( ap_cur->security & AUTH_OPN   ) fprintf( G.f_M4, " OPN");
        }

        fprintf( G.f_M4, ", %3d, %8ld, %8ld, ",
                 ap_cur->avg_power,
                 ap_cur->nb_bcn,
                 ap_cur->nb_data );

        fprintf( G.f_M4, "%3d.%3d.%3d.%3d, ",
                 ap_cur->lanip[0], ap_cur->lanip[1],
                 ap_cur->lanip[2], ap_cur->lanip[3] );

        fprintf( G.f_M4, "%3d, ", ap_cur->ssid_length);

        for(i=0; i<ap_cur->ssid_length; i++)
        {
            fprintf( G.f_M4, "%c", ap_cur->essid[i] );
        }
        fprintf( G.f_M4, ", " );


        fprintf( G.f_M4, "%d,", ap_cur->nb_dataps);

        if(ap_cur->key != NULL)
        {
            for(i=0; i<(int)strlen(ap_cur->key); i++)
            {
                fprintf( G.f_M4, "%02X", ap_cur->key[i]);
        ...
        fprintf( G.f_M4,
            ...
            "Power, # packets, BSSID,  Paquets lost, rate, Probed ESSIDs\r\n" );
    ...
    while( st_cur != NULL )
    {
        ...
        fprintf( G.f_M4, "%d, from: %d to: %d,",st_cur->missed, st_cur->rate_from/1000000,st_cur->rate_to/1000000);
...
//             printf("finished rearranging\n");
    ...
    //M4
    G.f_M4 = fopen(FILE_BUFFER_M4, "wt");
    if (G.f_M4 == NULL) {
        fprintf(stderr, "Error trying to open .m4 buffer");
    }
