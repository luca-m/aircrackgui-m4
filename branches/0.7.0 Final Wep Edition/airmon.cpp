...
airmon::airmon(QWidget *parent) :
    ...
    ui(new Ui::airmon)
{
    debug::add("Constructor of airmon object " + QString::number(this->winId()));
...
airmon::~airmon()
{
    debug::add("Destructor of airmon object " + QString::number(this->winId()));
...
void airmon::modeMonitorOn(){
    ...
    const QString command = AIRMON_COM + " start " + this->getInterfaceSelected();

    QProcess p;
    p.setProcessChannelMode(QProcess::MergedChannels);
    p.start(command);
    debug::add("Executing airmon (monitorModeON) process " + QString::number(p.pid()));
...
void airmon::modeMonitorOff(){
    ...
    const QString command = AIRMON_COM + " stop " + this->getInterfaceSelected();

    QProcess p;
    p.setProcessChannelMode(QProcess::MergedChannels);
    p.start(command);
    debug::add("Executing airmon (monitorModeOFF) process " + QString::number(p.pid()));
